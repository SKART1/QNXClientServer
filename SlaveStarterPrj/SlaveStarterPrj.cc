...
int pareseServerInfoFile(std::string pathToInfoFile, ArgSlaveStruct *argSlaveStruct){
	FILE *filePointer;
	int result=0;
	if((filePointer=fopen(pathToInfoFile.c_str(),"r"))==NULL){
		...
		result=result+fscanf(filePointer, "CHID_RESULTS_FOR_CLIENTS: %d\n", &argSlaveStruct->chidResults);
		fclose(filePointer);
		if(result!=4){
...
int main(int argc, char *argv[]) {
	...
	if(argc<2){
		...
		pareseServerInfoFile("serv.serv", argSlaveStruct);
	}
	else{
		pareseServerInfoFile(argv[1], argSlaveStruct);
	}
